...
const CONFIG = {
    ...
    MIN_CLAN_LEVEL: parseInt(process.env.MIN_CLAN_LEVEL) || 2,
...
async function trackNewMembers() {
    try {
        ...
        const activeClans = clans.filter(clan => 
            ...
            clan.clanLevel >= CONFIG.MIN_CLAN_LEVEL &&
            clan.clanPoints >= CONFIG.MIN_CLAN_POINTS
        );
        
        console.log(`Processing ${activeClans.length} active clans (min ${CONFIG.MIN_CLAN_MEMBERS} members, level ${CONFIG.MIN_CLAN_LEVEL}+, ${CONFIG.MIN_CLAN_POINTS}+ points)`);
        ...
        for (const clan of activeClans) {
            try {
                console.log(`Fetching details for ${clan.name} (${clan.tag}) - Level ${clan.clanLevel}, ${clan.members} members`);
                const clanData = await fetchClanDetails(clan.tag);
                
                // Filter members with 4000+ trophies and only regular members
                const qualifiedMembers = clanData.memberList.filter(
                    member => member.trophies >= CONFIG.MIN_TROPHIES && 
                             member.role === 'member'  // Only regular members
                );
...
function findNewMembers(state, clanData, qualifiedMembers) {
    ...
    return qualifiedMembers
        ...
        .map(m => ({
            ...
            tag: m.tag,
            trophies: m.trophies,
            joined: new Date(m.joined).toLocaleString()
        }));
}
        ...
        .then(updates => {
            ...
            updates.forEach(clan => {
                ...
                clan.members.forEach(member => {
                    console.log(`- ${member.name.padEnd(15)} ${member.trophies.toString().padStart(4)} trophies, joined ${member.joined}`);
                });
            });
        })
